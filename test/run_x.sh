...
FILE_BASE=${$1%.cpp}
echo "FILE_BASE: `$FILE_BASE`"  #DEBUG
...
if [[ $FILE_BASE == *".man" ]] then # this is a manual file
  ...
  $HIP_EXE -L$SCABBARD_PATH -ltrace -Ilib/include -Ilibtrace/include -Ilibtrace/src -std=c++17 -x hip -g -02 -o${FILE_BASE}.out $1
...
export SCABBARD_INSTRUMENTED_EXE_NAME="$(realpath ${FILE_BASE}.out)"
export SCABBARD_TRACE_FILE="${$SCABBARD_INSTRUMENTED_EXE_NAME%.out}.scabbard.trace"
