...
{
  using namespace scabbard;
  src_id = scabbard::trace::metadata_register$src("test/device/dual-stream/maybe-race.man.cpp");
  ...
  // syncing only one stream (especially the one created and launched first with priority) leaves you at risk for data races
  ...
  for (size_t i=0; i<DIM*DIM; ++i) {
    ...
    tmp1 = C2[((DIM*DIM)-1)-i]; // Going backwards for C2 can make it technically possible to get a data race when S2 is left unsynced (but always a warning)
    scabbard::trace::host::trace_append$mem(
        InstrData::READ | InstrData::ON_HOST | InstrData::DEVICE_HEAP,
        &(C2[((DIM*DIM)-1)-i]),
        &src_id, 115u, 5u
