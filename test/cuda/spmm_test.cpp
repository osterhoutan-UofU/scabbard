...
/**
 * @file spmm_test.cpp
 * @author https://github.com/hgyhungry/ge-spmm
 * @brief geSpMM a sparse matrix multiplication algorithm
 *        NOTE: This has been converted to hip code from cuda,
 *          However, it contains a hard coded assumption of a 
 *           warp size of 32 and AMD GPU's do not all have a 
 *           warp size of 32, some have 8, 16, or 64 
 *           (most being the latter, if they support hip/ROCm).
 *          Therefore, this does not work in hip even if you
 *           enable HIP_ENABLE_WARP_SYNC_BUILTINS, 
 *           which requires ROCm >=6.2, 
 *           but is disabled by default in ROCm <6.3.
 *          TLDR: this won't compile unless ROCm >=6.3 &
 *           even then it will not work as intended since
 *           warp sizes on AMD GPU's are likely to be anything but 32.
 * 
 */
...
do {                                    \
    ...
    if (A_data_dev) hipFree(A_data_dev);     \
    if (A_indptr_dev) hipFree(A_indptr_dev);         \
    if (A_indices_dev) hipFree(A_indices_dev);     \
    ...
    if (cusp_handle)     hipsparseDestroy(cusp_handle);    \
    fclose(fpo);\
    hipDeviceReset();                  \
    fflush(stdout);                     \
} while (0)
...
int main(int argc, char** argv) {
    ...
    hipsparseHandle_t cusp_handle=0;
    // hipblasHandle_t cubl_handle=0;
    hipsparseMatDescr_t descr=0;
    hipsparseStatus_t cusp_stat;
    // hipblasStatus_t cubl_stat;
    ...
    while (true) {
        cudaStat1 = hipMalloc((void**)&A_indptr_dev, (A_nrows+1)*sizeof(A_indptr_dev[0]));
        cudaStat2 = hipMalloc((void**)&A_indices_dev, nnz*sizeof(A_indices_dev[0]));
        cudaStat3 = hipMalloc((void**)&A_data_dev, nnz*sizeof(A_data_dev[0]));
        cudaStat4 = hipMalloc((void**)&B_dev, max_ncols*A_ncols*sizeof(B_dev[0]));
        cudaStat5 = hipMalloc((void**)&C_dev, A_nrows*max_ncols*sizeof(C_dev[0]));
        // cudaStat6 = hipMalloc((void**)&C_tran_dev, A_nrows*max_ncols*sizeof(C_tran_dev[0]));
        if ((cudaStat1 == hipSuccess) && (cudaStat2 == hipSuccess) && (cudaStat3 == hipSuccess) &&
        (cudaStat4 == hipSuccess) && (cudaStat5 == hipSuccess)) {
    ...
    checkCudaError(hipMemcpy(A_indptr_dev, A_indptr, (A_nrows+1)*sizeof(A_indptr_dev[0]), hipMemcpyHostToDevice));
    checkCudaError(hipMemcpy(A_indices_dev, A_indices, nnz*sizeof(A_indices_dev[0]), hipMemcpyHostToDevice));
    checkCudaError(hipMemcpy(A_data_dev, A_data, nnz*sizeof(A_data_dev[0]), hipMemcpyHostToDevice));
    checkCudaError(hipMemcpy(B_dev, B, max_ncols*A_ncols*sizeof(B_dev[0]), hipMemcpyHostToDevice));
    ...
    {
        fprintf(stderr, "Warm-up failed: %s\t", hipGetErrorString(cudaStat));
    }

    // init cusparse params
    // hipblasCreate(&cubl_handle);
    // hipblasSetPointerMode(cubl_handle, HIPBLAS_POINTER_MODE_HOST);
    hipsparseCreate(&cusp_handle);
    hipsparseCreateMatDescr(&descr);
    hipsparseSetMatType(descr, HIPSPARSE_MATRIX_TYPE_GENERAL);
    hipsparseSetMatIndexBase(descr, HIPSPARSE_INDEX_BASE_ZERO);

    cusp_stat = hipsparseScsrmm2(cusp_handle, HIPSPARSE_OPERATION_NON_TRANSPOSE, HIPSPARSE_OPERATION_TRANSPOSE, A_nrows, max_ncols, A_ncols, nnz, &one, descr, A_data_dev, A_indptr_dev, A_indices_dev, B_dev, max_ncols, &zero, C_dev, A_nrows);
    if (cusp_stat != HIPSPARSE_STATUS_SUCCESS) {
        CLEANUP("csrmm2 failed");
        return 1;
    }
    // cubl_stat = hipblasSgeam(cubl_handle, HIPBLAS_OP_T, HIPBLAS_OP_N, max_ncols, A_nrows, &one, C_dev, A_nrows, &zero, nullptr, max_ncols, C_tran_dev, max_ncols);
    // if (cubl_stat != HIPBLAS_STATUS_SUCCESS) {
...
#ifdef VALIDATE
    B_ncols=max_ncols;
    checkCudaError(hipMemcpy(C, C_dev, A_nrows*B_ncols*sizeof(C[0]), hipMemcpyDeviceToHost));
    for (int i=0; i<A_nrows; i++) 
    ...
    // checkCudaError(hipMemcpy(C, C_tran_dev, A_nrows*B_ncols*sizeof(C[0]), hipMemcpyDeviceToHost));
    // for (int i=0; i<A_nrows; i++) 
    //     for (int j=0; j<B_ncols; j++) 
    ...
    for (int method=0; method<5; method++) {
        checkCudaError(hipMemset((void*)C_dev, 0, A_nrows*B_ncols*sizeof(C_dev[0])));
...
#define ITER 200
    ...
    for (B_ncols=128; B_ncols<=max_ncols; B_ncols *= 2) {
        ...
        for (int i=0; i<ITER; i++) {
            cusp_stat = hipsparseScsrmm2(cusp_handle, HIPSPARSE_OPERATION_NON_TRANSPOSE, HIPSPARSE_OPERATION_TRANSPOSE, A_nrows, B_ncols, A_ncols, nnz, &one, descr, A_data_dev, A_indptr_dev, A_indices_dev, B_dev, B_ncols, &zero, C_dev, A_nrows);
        }
        hipEventRecord(stop, 0);
        hipEventSynchronize(stop);
        hipEventElapsedTime(&rt, start, stop);
        // fprintf(fpo, "%f,%f,", rt/ITER, gflop/(rt/ITER));
        fprintf(fpo, "%f,", gflop/(rt/ITER));
        // hipEventRecord(start, 0);
        // for (int i=0; i<ITER; i++) {
        //     cubl_stat = hipblasSgeam(cubl_handle, HIPBLAS_OP_T, HIPBLAS_OP_N, B_ncols, A_nrows, &one, C_dev, A_nrows, &zero, nullptr, B_ncols, C_tran_dev, B_ncols);
        // }
        // hipEventRecord(stop, 0);
        // hipEventSynchronize(stop);
        // hipEventElapsedTime(&rt2, start, stop);
        // rt += rt2;
        // // fprintf(fpo, "%f,%f,", rt/ITER, gflop/(rt/ITER));
        // fprintf(fpo, "%f,", gflop/(rt/ITER));
