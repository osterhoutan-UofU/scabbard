...
struct blockId_t {
  ...
  //   z = blockIdx.z;
  // }
  __host__ __device__ blockId_t(const dim3& blockId)
    : x(blockId.x),  
      y(blockId.y),
      z(blockId.z)
    {}
...
struct threadId_t {
  ...
  // }
  __device__
  __host__ inline threadId_t(const dim3& threadId)
    : x(threadId.x),  
      y(threadId.y),
      z(threadId.z)
    {}
...
struct DeviceThreadId {
  ...
  __host__ inline DeviceThreadId(const dim3& blockId, const dim3& threadId) 
...
union ThreadId {
  ...
  void* _NONE_;
  __device__ inline ThreadId(const dim3& blockId_, const dim3& threadId_) { device = DeviceThreadId(blcokId_, threadId_); }
  __host__ ThreadId() { std::memset(this,0,sizeof(ThreadId)); this->host = ::std::this_thread::get_id(); }
  __host__ __device__ ThreadId(void* _) { std::memset(this,0,sizeof(ThreadId)); }
...
struct TraceData {
  ...
  __host__ TraceData(InstrData data_, const ThreadId& threadId_,
    ...
    : data(data_), threadId(threadId_),
      ptr(ptr_), metadata(metadata_), time_stamp(time_stamp_)
  {}
  __host__ __device__ inline bool empty() const { return data == InstrData::NEVER; }
  // Explicit operators
